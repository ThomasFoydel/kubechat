...
async function authenticate(
  ...
  return getUserIdFromCookieHeader(
...
async function refreshNodeLeases(): Promise<void> {
  const conversations =
    connectionManager.getSubscribedConversationIds()
...
export async function initializeWebSocketPubSub(): Promise<void> {
  ...
  leaseRefreshInterval =
    setInterval(() => {
      void refreshNodeLeases().catch(
        error => {
          console.error(
            'WebSocket lease refresh error:',
            error
          )
        }
      )
