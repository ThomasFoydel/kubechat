...
const websocketChannelPrefix =
  'kubechat:websocket:node:'

const conversationNodesPrefix =
  'kubechat:websocket:conversation:'
...
function getConversationNodesKey(
  conversationId: string
): string {
...
export async function initializeRedisPubSub(
  ...
  await subscriber.subscribe(
    ...
    message => {
      try {
        const event =
          JSON.parse(message) as WebSocketEvent
        ...
        console.error(
          'Failed to process Redis WebSocket event:',
          error
        )
      }
    }
...
export async function registerConversationNode(
  conversationId: string
): Promise<void> {
  await publisher.zAdd(
    getConversationNodesKey(
      conversationId
    ),
    {
...
export async function unregisterConversationNode(
  conversationId: string
): Promise<void> {
  await publisher.zRem(
    ...
    config.websocketNodeId
  )
}

export async function refreshConversationNodeLease(
  conversationId: string
): Promise<void> {
  await publisher.zAdd(
    getConversationNodesKey(
