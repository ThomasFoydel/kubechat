...
describe('POST /api/v1/auth/register', () => {
  ...
  it('registers a user and sets a session cookie', async () => {
    ...
    expect(response.headers['set-cookie']).toEqual(
      expect.arrayContaining([expect.stringContaining(`kubechat_session=${sessionId}`)]),
    )
...
describe('POST /api/v1/auth/logout', () => {
  ...
  it('logs out the current session and clears the cookie', async () => {
    ...
    const response = await request(app)
      ...
      .set('Cookie', `kubechat_session=${sessionId}`)
    ...
    expect(response.headers['set-cookie']).toEqual(
      expect.arrayContaining([expect.stringContaining('kubechat_session=')]),
    )
  })
...
describe('GET /api/v1/auth/me', () => {
  ...
  it('returns the current user for a valid session', async () => {
    ...
    vi.mocked(getUserIdFromSession).mockResolvedValue(user.id)
    ...
    expect(getUserIdFromSession).toHaveBeenCalledWith(sessionId)
  ...
  it('rejects a session for a user that no longer exists', async () => {
    ...
    vi.mocked(getUserIdFromSession).mockResolvedValue(deletedUserId)
